...
export default function SideCard({ gameData, onAmountChange, onDifficultyChange, onPlayDragon, difficultyLevels, onCashout }) {
    ...
    <Tabs defaultValue="manual" className="flex justify-center items-center flex-col p-3 gap-3">
      ...
      <TabsContent value="manual">
        <div className="grid items-center gap-5">
          ...
          <div className="flex gap-2 flex-col">
            <Label htmlFor="difficulty">Mines</Label>
                      ...
                      {difficultyLevels.map((diff) => (
                        <CommandItem
                          ...
                          onSelect={(currentValue) => {
                            onDifficultyChange(currentValue);
                            setOpen(false);
                          }}
                        >
                          <Check className={cn("mr-2 h-4 w-4", gameData["difficulty"] == diff ? "opacity-100" : "opacity-0")} />
                          {diff}
                        </CommandItem>
                      ))}
                    </CommandGroup>
          ...
          {gameData.dragonId && (
            <Button className="transition-all duration-150" onClick={onCashout} disabled={gameData.payoutMultiplier == 0}>
              Cashout
            </Button>
          )}
        </div>
